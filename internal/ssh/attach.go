...
func remoteAttachScript(name, cwd string, prov session.Provider, nativeID string, spec provider.ResumeSpec) string {
	...
	externalAttach := strings.Join([]string{
		"/bin/sh",
		"-c",
		quotePOSIX(arsruntime.ExternalAttachScript()),
		"ars-external-attach",
		`"$socket"`,
		`"$pane"`,
		`"$pane_pid"`,
	...
	return strings.Join([]string{
		...
		"      socket_type=",
		"      extra=",
		"      IFS=\"$tab\" read -r kind socket pane pane_pid socket_dev socket_inode socket_uid socket_type extra <<EOF",
		"$external_result",
		"EOF",
		"      [ \"$kind\" = match ] || exit 1",
		"      [ -n \"$socket\" ] && [ -n \"$pane\" ] && [ -n \"$pane_pid\" ] || exit 1",
		"      [ -n \"$socket_dev\" ] && [ -n \"$socket_inode\" ] && [ -n \"$socket_uid\" ] || exit 1",
		"      [ \"$socket_type\" = socket ] && [ -z \"$extra\" ] || exit 1",
		"      exec " + externalAttach,
		"      ;;",
		"    *)",
